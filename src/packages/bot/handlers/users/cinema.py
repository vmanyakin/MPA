...
from src.packages.bot.loader import bot, dispatcher, load_text_messages, template_json, load_films, load_serials, load_cartoons, load_anime


@dispatcher.callback_query_handler(text="Кинопоиск")
async def cinema_start(call: types.CallbackQuery):
    text_cinema_start = template_json(load_text_messages["cinema"]["cinema_start"] + load_text_messages["cinema"]["label"]).render()
    await call.message.answer(text_cinema_start, reply_markup=cinema_data_keyboard)


@dispatcher.callback_query_handler(lambda callback_query: callback_query.data in ["Фильм", "Сериал", "Мультфильм", "Аниме"])
async def cinema_data(call: types.CallbackQuery):
    ...
    text_cinema_data = template_json(
        load_text_messages["cinema"]["cinema_data"] + load_text_messages["cinema"]["label"]).render(title=title,genre=genre,director=director,country=country, year=year,duration=duration,rating=rating,trailer=trailer)
